...
export function drawLines() {
  ...
  Object.entries(data.groups).forEach(([_, group]: [string, Group]) => {
    group.nodes.forEach((nodeId: string, _idx: number) => {
      ...
      if (node.out && !node.isMastery && !node.classStartIndex) {
        node.out.forEach((targetId: string) => {
          ...
          const key =
            node.skill < target.skill
              ? `${node.skill}-${target.skill}`
              : `${target.skill}-${node.skill}`;
          if ((window as unknown as { drawn?: Set<string> }).drawn?.has(key))
            return;
          (window as unknown as { drawn?: Set<string> }).drawn =
            (window as unknown as { drawn?: Set<string> }).drawn || new Set();
          (window as unknown as { drawn?: Set<string> }).drawn!.add(key);

<script lang="ts">
  interface Props {
    onclick: () => void;
  }

  let { onclick }: Props = $props();
  let showTooltip = $state(false);
</script>

<div class="relative inline-flex">
  <button
    type="button"
    {onclick}
    class="p-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded cursor-pointer transition-all duration-200 shadow-indigo-500/20"
    aria-label="Copy for POB"
    onmouseenter={() => (showTooltip = true)}
    onmouseleave={() => (showTooltip = false)}
    onfocus={() => (showTooltip = true)}
    onblur={() => (showTooltip = false)}
  >
    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
        d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3"
      />
    </svg>
  </button>

  {#if showTooltip}
    <div
      class="absolute left-1/2 -translate-x-1/2 top-full mt-2 px-2 py-1 bg-slate-800 text-white text-xs rounded whitespace-nowrap z-50 shadow-lg border border-slate-600 pointer-events-none"
      role="tooltip"
    >
      Copy for POB
      <div
        class="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-slate-800 border-t border-l border-slate-600 rotate-45"
      ></div>
    </div>
  {/if}
</div>
